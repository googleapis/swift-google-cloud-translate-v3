...
func sample(
  ...
  let response = try await client.updateGlossaryEntry(
    request: UpdateGlossaryEntryRequest()
      .with {
        $0.glossaryEntry = GlossaryEntry().with {
          $0.name =
            "projects/\(projectId)/locations/\(locationId)/glossaries/\(glossaryId)/glossaryEntries/\(glossaryEntryId)"
        }
      }
  )
  ...
  static func main() async throws {
    do {
      ...
      try await sample(
        ...
        glossaryId: "[placeholder]", glossaryEntryId: "[placeholder]")
    } catch {
      print("Error: \(error)")
    }
  }
