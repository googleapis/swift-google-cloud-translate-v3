...
```bash
swift package add-dependency https://github.com/googleapis/swift-google-cloud-translate-v3.git --from 0.1.0-preview
```

Then add `GoogleCloudTranslateV3` to your target's dependencies:

```bash
swift package add-target-dependency GoogleCloudTranslateV3 <target-name> --package swift-google-cloud-translate-v3
```
